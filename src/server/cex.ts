...
/** Convert CEX's box-detail wire payload into the import boundary shape. */
export function parseCexResponse(
  payload: CexResponse,
  barcode: string
): CexImport | null {
  const box = payload.response?.data?.boxDetails?.[0]
  if (!box?.boxName) return null
